...
NR_UE_L2_STATE_t nr_ue_scheduler(nr_downlink_indication_t *dl_info, nr_uplink_indication_t *ul_info) {
  ...
  if ((mac->scheduling_info.periodicBSR_SF !=
       MAC_UE_BSR_TIMER_NOT_RUNNING)
...
boolean_t
nr_update_bsr(module_id_t module_idP, frame_t frameP,
  ...
  for (lcid=DCCH; lcid < NR_MAX_NUM_LCID; lcid++) {
    ...
    if (mac->logicalChannelBearer_exist[lcid] ) { // todo
      ...
      rlc_status = mac_rlc_status_ind(module_idP, mac->crnti,gNB_index,frameP,slotP,ENB_FLAG_NO,MBMS_FLAG_NO,
                                      lcid,
                                      0, 0
                                     );
  ...
  for (lcid = DCCH; lcid < NR_MAX_NUM_LCID; lcid++) {
    mac->scheduling_info.LCID_buffer_remain[lcid] =
          ...
          if ( (s_map>>map_shift)&0x01 ) {
            ...
            for (n_prach_occ_in_time=0; n_prach_occ_in_time<N_t_slot; n_prach_occ_in_time++) {
              ...
              for (n_prach_occ_in_freq=0; n_prach_occ_in_freq<nb_fdm; n_prach_occ_in_freq++) {
                prach_occasion_info_t *prach_occasion_p =
                  &prach_assoc_pattern.prach_conf_period_list[prach_conf_period_idx].prach_occasion_slot_map[prach_conf_period_frame_idx][slot].prach_occasion[n_prach_occ_in_time][n_prach_occ_in_freq];
...
static void map_ssb_to_ro(NR_UE_MAC_INST_t *mac) {
  ...
  prach_assoc_pattern.prach_association_period_list[0].nb_of_frame = prach_assoc_pattern.prach_association_period_list[0].nb_of_prach_conf_period *
...
uint8_t nr_ue_get_sdu(module_id_t module_idP,
  ...
  if ((mac->scheduling_info.periodicBSR_Timer !=
  ...
  for (lcg_id = 0; lcg_id < NR_MAX_NUM_LCGID; lcg_id++) {
    if (bsr_ce_len == sizeof(NR_BSR_SHORT)) {
      ...
      mac->scheduling_info.BSR[lcg_id] =
        nr_locate_BsrIndexByBufferSize(NR_LONG_BSR_TABLE, NR_LONG_BSR_TABLE_SIZE,
                                       mac->scheduling_info.
                                       BSR_bytes[lcg_id]);
    }
