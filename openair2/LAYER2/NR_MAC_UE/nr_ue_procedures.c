  ...
  switch(dci_format) {
    ...
    case NR_DL_DCI_FORMAT_1_0: {
      ...
      if (mac->ULbwp[0] &&
        ...
        for (int id = 0; id < pucch_res_set_cnt; id++) {
          if (dci->pucch_resource_indicator <
              mac->cg->spCellConfig->spCellConfigDedicated->uplinkConfig->initialUplinkBWP->pucch_Config->choice.setup->resourceSetToAddModList->list.array[id]->resourceList.list.count) {
            valid = 1;
            break;
          }
        }
    ...
    case NR_DL_DCI_FORMAT_1_1: {
      ...
      dlsch_config_pdu_1_1->dlDmrsSymbPos = fill_dmrs_mask(pdsch_config,
                                            ...
                                            mappingtype);
      dlsch_config_pdu_1_1->dmrsConfigType = mac->DLbwp[dl_bwp_id-1]->bwp_Dedicated->pdsch_Config->choice.setup->dmrs_DownlinkForPDSCH_MappingTypeA->choice.setup->dmrs_Type == NULL ? NFAPI_NR_DMRS_TYPE1 :
                                             NFAPI_NR_DMRS_TYPE2;
