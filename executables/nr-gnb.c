...
void init_gNB(int single_thread_flag,int wait_for_sync) {
  ...
  for (inst=0; inst<RC.nb_nr_L1_inst; inst++) {
    ...
    gNB->UL_INFO.rx_ind.pdu_list = calloc(NFAPI_RX_IND_MAX_PDU,sizeof(nfapi_rx_indication_pdu_t));
    gNB->UL_INFO.crc_ind.crc_list = calloc(NFAPI_CRC_IND_MAX_PDU,sizeof(nfapi_crc_indication_pdu_t));
    gNB->UL_INFO.srs_ind.pdu_list = calloc(NFAPI_SR_IND_MAX_PDU,sizeof(nfapi_sr_indication_pdu_t));
