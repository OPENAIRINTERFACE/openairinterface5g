...
void gNB_top(PHY_VARS_gNB *gNB, int frame_rx, int slot_rx, struct RU_t_s *ru) {
  ...
  if (!oai_exit) {
    ...
    L1_proc->frame_tx     = (L1_proc->slot_rx > (fp->slots_per_frame-1-sl_ahead)) ?
...
void init_gNB(int single_thread_flag,int wait_for_sync) {
  ...
  AssertFatal(RC.gNB != NULL,"");
  for (inst=0; inst<RC.nb_nr_L1_inst; inst++) {
    AssertFatal(RC.gNB[inst] != NULL,"");

    for (CC_id=0; CC_id<RC.nb_nr_L1_CC[inst]; CC_id++) {
      AssertFatal(RC.gNB[inst][CC_id] != NULL,"");
      gNB                     = RC.gNB[inst][CC_id];
...
void rx_rf(RU_t *ru,int *frame,int *slot) {
  ...
  if (proc->first_rx == 1) {
    ...
    proc->timestamp_rx = 0;
    AssertFatal(proc->tti_rx == *slot && proc->frame_rx == *frame ,
		"Received Timestamp (%llu) doesn't correspond to the time we think it is \n\
(proc->tti_rx %d, subframe %d, proc->frame_rx %d frame %d)\n",
		(long long unsigned int)proc->timestamp_rx,proc->tti_rx,*slot, proc->frame_rx,*frame);
  } else {
...
static void *ru_thread( void *param ) {
  ...
  while (!oai_exit) {
    ...
    //ru->feptx_ofdm
    nfapi_nr_config_request_t *cfg = &ru->gNB_list[0]->gNB_config;
    if (nr_slot_select(cfg,proc->tti_tx,proc->frame_tx ) != SF_UL) {
