...
void nrLDPC_cnProc_BG1_generator_AVX512(int R)
{
 ...
 // system("mkdir -p ../ldpc_gen_files");
  ...
  fprintf(fd,"static inline void nrLDPC_cnProc_BG1_R%s_AVX512(t_nrLDPC_lut* p_lut, t_nrLDPC_procBuf* p_procBuf, uint16_t Z) {\n",ratestr[R]);
  ...
  fprintf(fd,"                __m512i* p_cnProcBufResBit;\n");
  fprintf(fd,"                __m512i* p_cnProcBufRes;\n");
  fprintf(fd,"                __m512i* p_cnProcBuf;\n");
  ...
  if (lut_numCnInCnGroups[0] > 0)
    {
      ...
      bitOffsetInGroup = (lut_numCnInCnGroups[0]*NR_LDPC_ZMAX)>>6;
      fprintf(fd,"  p_cnProcBufRes = (__m512i*) &cnProcBufRes[%d];\n", lut_startAddrCnGroups[0]);
       fprintf(fd,"  p_cnProcBuf = (__m512i*) &cnProcBuf[%d];\n", lut_startAddrCnGroups[0]);

     // Loop over every BN

      for (j=0; j<3; j++)
        {
fprintf(fd,"   p_cnProcBufResBit = p_cnProcBufRes + %d ;\n", (j*bitOffsetInGroup));
          fprintf(fd,"            for (i=0;i<M;i++) {\n");
          // Abs and sign of 64  CNs (first BN)
          //                zmm0 = p_cnProcBuf[lut_idxCnProcG3[j][0] + i];
            fprintf(fd,"                zmm0 = p_cnProcBuf[%d+i];\n",lut_idxCnProcG3[j][0]/2);
         ...
         // for (k=1; k<2; k++)
          ...
          //                min = _mm512_min_epu8(min, *maxLLR); // 128 in epi8 is -127
            ...
            fprintf(fd,"                *p_cnProcBufResBit = conditional_negate(min, sgn,zeros);\n");
            fprintf(fd,"                p_cnProcBufResBit++;\n");
            fprintf(fd,"            }\n");
        }
  ...
  if (lut_numCnInCnGroups[1] > 0)
    {
      ...
      // Set the offset to each bit within a group in terms of 64  Byte
      bitOffsetInGroup = (lut_numCnInCnGroups[1]*NR_LDPC_ZMAX)>>6;
       fprintf(fd,"  p_cnProcBufRes = (__m512i*) &cnProcBufRes[%d];\n", lut_startAddrCnGroups[1]);
       fprintf(fd,"  p_cnProcBuf = (__m512i*) &cnProcBuf[%d];\n", lut_startAddrCnGroups[1]);
  ...
  if (lut_numCnInCnGroups[2] > 0)
    {
      ...
      // Set the offset to each bit within a group in terms of 64  Byte
      bitOffsetInGroup = (lut_numCnInCnGroups[2]*NR_LDPC_ZMAX)>>6;
       fprintf(fd,"  p_cnProcBufRes = (__m512i*) &cnProcBufRes[%d];\n", lut_startAddrCnGroups[2]);
       fprintf(fd,"  p_cnProcBuf = (__m512i*) &cnProcBuf[%d];\n", lut_startAddrCnGroups[2]);
  ...
  if (lut_numCnInCnGroups[3] > 0)
    {
      ...
      // Set the offset to each bit within a group in terms of 64  Byte
      bitOffsetInGroup = (lut_numCnInCnGroups[3]*NR_LDPC_ZMAX)>>6;
       fprintf(fd,"  p_cnProcBufRes = (__m512i*) &cnProcBufRes[%d];\n", lut_startAddrCnGroups[3]);
       fprintf(fd,"  p_cnProcBuf = (__m512i*) &cnProcBuf[%d];\n", lut_startAddrCnGroups[3]);
  ...
  if (lut_numCnInCnGroups[4] > 0)
    {
     ...
     fprintf(fd, "M = (%d*Z + 63)>>6;\n",lut_numCnInCnGroups[4] );
      ...
      // Set the offset to each bit within a group in terms of 64  Byte
      bitOffsetInGroup = (lut_numCnInCnGroups[4]*NR_LDPC_ZMAX)>>6;
       fprintf(fd,"  p_cnProcBufRes = (__m512i*) &cnProcBufRes[%d];\n", lut_startAddrCnGroups[4]);
       fprintf(fd,"  p_cnProcBuf = (__m512i*) &cnProcBuf[%d];\n", lut_startAddrCnGroups[4]);
  ...
  if (lut_numCnInCnGroups[5] > 0)
    {
     ...
     fprintf(fd, "M = (%d*Z + 63)>>6;\n",lut_numCnInCnGroups[5] );
      ...
      // Set the offset to each bit within a group in terms of 64  Byte
      bitOffsetInGroup = (lut_numCnInCnGroups[5]*NR_LDPC_ZMAX)>>6;
       fprintf(fd,"  p_cnProcBufRes = (__m512i*) &cnProcBufRes[%d];\n", lut_startAddrCnGroups[5]);
       fprintf(fd,"  p_cnProcBuf = (__m512i*) &cnProcBuf[%d];\n", lut_startAddrCnGroups[5]);
  ...
  if (lut_numCnInCnGroups[6] > 0)
    {
     ...
     fprintf(fd, "M = (%d*Z + 63)>>6;\n",lut_numCnInCnGroups[6] );
      ...
      // Set the offset to each bit within a group in terms of 64  Byte
      bitOffsetInGroup = (lut_numCnInCnGroups[6]*NR_LDPC_ZMAX)>>6;
       fprintf(fd,"  p_cnProcBufRes = (__m512i*) &cnProcBufRes[%d];\n", lut_startAddrCnGroups[6]);
       fprintf(fd,"  p_cnProcBuf = (__m512i*) &cnProcBuf[%d];\n", lut_startAddrCnGroups[6]);
  ...
  if (lut_numCnInCnGroups[7] > 0)
    {
      ...
      // Set the offset to each bit within a group in terms of 64  Byte
      bitOffsetInGroup = (lut_numCnInCnGroups[7]*NR_LDPC_ZMAX)>>6;
       fprintf(fd,"  p_cnProcBufRes = (__m512i*) &cnProcBufRes[%d];\n", lut_startAddrCnGroups[7]);
       fprintf(fd,"  p_cnProcBuf = (__m512i*) &cnProcBuf[%d];\n", lut_startAddrCnGroups[7]);
  ...
  if (lut_numCnInCnGroups[8] > 0)
    {
     ...
     fprintf(fd, " M = (%d*Z + 63)>>6;\n",lut_numCnInCnGroups[8] );
      ...
      // Set the offset to each bit within a group in terms of 64  Byte
      bitOffsetInGroup = (lut_numCnInCnGroups[8]*NR_LDPC_ZMAX)>>6;
       fprintf(fd,"  p_cnProcBufRes = (__m512i*) &cnProcBufRes[%d];\n", lut_startAddrCnGroups[8]);
       fprintf(fd,"  p_cnProcBuf = (__m512i*) &cnProcBuf[%d];\n", lut_startAddrCnGroups[8]);
