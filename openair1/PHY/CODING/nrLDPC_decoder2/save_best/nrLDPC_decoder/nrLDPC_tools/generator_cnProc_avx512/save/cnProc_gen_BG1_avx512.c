...
void nrLDPC_cnProc_BG1_generator_AVX512(uint16_t Z,int R)
{
 ...
 // system("mkdir -p ../ldpc_gen_files");
  ...
  char fname[50];
  sprintf(fname,"../ldpc_gen_files/nrLDPC_cnProc_BG1_Z%d_R%s_AVX512.c",Z,ratestr[R]);
  ...
  fprintf(fd,"void nrLDPC_cnProc_BG1_Z%d_R%s_AVX512(t_nrLDPC_lut* p_lut, t_nrLDPC_procBuf* p_procBuf) {\n",Z,ratestr[R]);
  ...
  fprintf(fd,"                  const __m512i* p_ones   = (__m512i*) ones512_epi8;\n");   
      ...
      for (j=0; j<3; j++)
        {
          ...
          //                zmm0 = p_cnProcBuf[lut_idxCnProcG3[j][0] + i];
            ...
            fprintf(fd,"                sgn  = _mm512_xor_si512(*p_ones, zmm0);\n");
