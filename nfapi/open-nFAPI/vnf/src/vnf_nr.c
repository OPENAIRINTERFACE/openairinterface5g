...
void vnf_nr_handle_pnf_param_response(void *pRecvMsg, int recvMsgLen, nfapi_nr_vnf_config_t *config, int p5_idx)
{
...
void vnf_nr_handle_pnf_config_response(void *pRecvMsg, int recvMsgLen, nfapi_nr_vnf_config_t *config, int p5_idx)
{
  if (pRecvMsg == NULL || config == NULL) {
    NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s: NULL parameters\n", __FUNCTION__);
  } else {
...
void vnf_nr_handle_pnf_start_response(void *pRecvMsg, int recvMsgLen, nfapi_nr_vnf_config_t *config, int p5_idx)
{
  if (pRecvMsg == NULL || config == NULL) {
    NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s: NULL parameters\n", __FUNCTION__);
  } else {
...
void vnf_nr_handle_pnf_stop_response(void *pRecvMsg, int recvMsgLen, nfapi_nr_vnf_config_t *config, int p5_idx)
{
  UNUSED(p5_idx);
  if (pRecvMsg == NULL || config == NULL) {
    NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s: NULL parameters\n", __FUNCTION__);
...
void vnf_nr_handle_param_response(void *pRecvMsg, int recvMsgLen, nfapi_nr_vnf_config_t *config, int p5_idx)
{
  if (pRecvMsg == NULL || config == NULL) {
    NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s: NULL parameters\n", __FUNCTION__);
  } else {
    ...
    if (result) {
      if (msg.error_code == NFAPI_NR_PARAM_MSG_OK) {
        nfapi_vnf_phy_info_t *phy_info = nfapi_nr_vnf_phy_info_list_find(config, msg.header.phy_id);
        if (phy_info) {
...
void vnf_nr_handle_config_response(void *pRecvMsg, int recvMsgLen, nfapi_nr_vnf_config_t *config, int p5_idx)
{
  if (pRecvMsg == NULL || config == NULL) {
    NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s: NULL parameters\n", __FUNCTION__);
  } else {
...
void vnf_nr_handle_start_response(void *pRecvMsg, int recvMsgLen, nfapi_nr_vnf_config_t *config, int p5_idx)
{
  if (pRecvMsg == NULL || config == NULL) {
    NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s: NULL parameters\n", __FUNCTION__);
  } else {
...
void vnf_nr_handle_error_indication(void *pRecvMsg, int recvMsgLen, nfapi_nr_vnf_config_t *config, int p5_idx)
{
  if (pRecvMsg == NULL || config == NULL) {
    NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s: NULL parameters\n", __FUNCTION__);
  } else {
...
void vnf_nr_handle_stop_indication(void *pRecvMsg, int recvMsgLen, nfapi_nr_vnf_config_t *config, int p5_idx)
{
  if (pRecvMsg == NULL || config == NULL) {
    NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s: NULL parameters\n", __FUNCTION__);
  } else {
  ...
  switch (messageHeader.message_id) {
    ...
    default:
      if (messageHeader.message_id >= NFAPI_VENDOR_EXT_MSG_MIN && messageHeader.message_id <= NFAPI_VENDOR_EXT_MSG_MAX) {
        vnf_nr_handle_vendor_extension(pRecvMsg, recvMsgLen, config, p5_idx, messageHeader.message_id);
      } else {
