...
//printf("init op data len %d data %x seg len %d\n",m_head->data_len, *data,seg->length);
if (i==0)
printf("init op data  seg len %d data %x\n",seg->length,*seg->addr);
...
printf("init op data len %d data %x\n",m_head->data_len, *data);
...
if (i==0)
{
printf("type input len %d nb seg %d\n",seg->length,ref_entries->nb_segments);

//printf("mhead len %d data %x\n",m_head->data_len, *data);
//printf("mhead len %d \n",m_head->data_len );
for (lc =0; lc<8; lc++)
{
printf("**********mhead len %d data %x addr %d\n",m_head->data_len, *(data+lc),(data+lc));
}
}
...
printf("loop rte memcpy data length %d\n",seg->length);
/*if (i==0)
{
for (lc =0; lc<8; lc++)
{
...
if (i==0)
{
printf("rte memcpy data length %d\n",seg->length);
if (i==0)
for (lc =0; lc<8; lc++)
{
printf("mhead len %d data %x addr %d\n",m_head->data_len, *(data+lc),(data+lc));
...
printf("start llr scaling \n");
	...
	for (i = 0; i < n; ++i) {
		struct rte_mbuf *m = input_ops[i].data;
char * addrl = &(m->buf_addr);
unsigned int off=256; //m->data_off;
unsigned int ll;
if (i==0){
for (ll=0; ll<8;ll++)
printf("llr data len %d data %x addr %d offset %d data off %d\n",m->data_len,*(addrl+ll+off),(addrl+ll+off),input_ops[i].offset,m->data_off);
}
...
if (i==0){
printf("addrllr %d llr %x llr orig addr %d data %x\n",addrllr, *addrllr,llr,*llr);}	
...
//struct rte_bbdev_op_data *bufs = *queue_ops[type];
struct rte_mbuf *m_head = rte_pktmbuf_alloc(mbuf_pools[type]);
//m_head = queue_ops[type][0]->data;	


//printf("queue ops len %d\n",m_head->data_len);

for (type = DATA_INPUT; type < DATA_NUM_TYPES; ++type) {
		struct op_data_entries *ref_entries =
				&test_vector.entries[type];
unsigned int nb_ops; 
nb_ops = ref_entries->nb_segments;
//printf("ref seg data %x \n", *ref_entries->segments[0].addr);
	
printf("ref seg  nb seg %d\n", nb_ops);
...
//printf("queue ops len %d\n",bufs[0].data->data_len);

//printf("queue ops len %d\n",queue_ops[type][0]->data->data_len);
	return 0;
}
...
char *addr = m->buf_addr;
printf("input data ad %d data %x len %d\n",(m->buf_addr),*addr,m->data_len);   
	for (i = 0; i < n; ++i) {
...
printf("cb param e %d\n",ldpc_dec->cb_params.e);
		...
		ops[i]->ldpc_dec.n_filler = 0; //ldpc_dec->n_filler;
...
//if (i<10)
//printf("ldpc_dec input %x\n",*ops[i]->ldpc_dec.input[0]->data->buf_addr);
...
create_reference_ldpc_dec_op(struct rte_bbdev_dec_op *op)
{
	...
	for (i = 0; i < entry->nb_segments; ++i)
		op->ldpc_dec.input.length +=
				entry->segments[i].length;
printf("data %x length %d\n",*entry->segments[0].addr,entry->segments[0].length);
...
printf("calc tb c %d r %d sys cols %d tb_size %d\n",op->ldpc_dec.tb_params.c, op->ldpc_dec.tb_params.r,sys_cols,tb_size);
	} else {
...
printf("start tp intr lcore ldpc dec\n");
	...
	printf("do enqueue ldpc dec ops\n");
...
unsigned int lm;
for (lm=0; lm<16;lm++ ){
printf("bufs len %d data %x addr %d\n",bufs->inputs[0].data->data_len,*(addrb+lm),(addrb+lm));
}

//printf("bufs len %d data %x\n",bufs->inputs[0].data->data_len,*bufs->inputs[0].data->buf_addr);
	...
	for (i = 0; i < TEST_REPETITIONS; ++i) {
		...
		start_time = rte_rdtsc_precise();
//printf("num ops %d burst sz %d enq %d\n",num_ops,burst_sz,enq);
		for (enq = 0, deq = 0; enq < num_ops;) {
			num_to_enq = burst_sz;
			...
			if (unlikely(num_ops - enq < num_to_enq))
				num_to_enq = num_ops - enq;
//if (enq==0)
//printf("pmd lcore ldpc dec length %d data %x\n", ops_enq[enq]->ldpc_dec.input.length,ops_enq[enq]->ldpc_dec.input);
...
{
printf("start tp lcore ldpc enc\n");
	struct thread_params *tp = arg;
	uint16_t enq, deq;
	uint64_t total_time = 0, start_time;
	const uint16_t queue_id = tp->queue_id+3;
	const uint16_t burst_sz = tp->op_params->burst_sz;
	const uint16_t num_ops = tp->op_params->num_to_process;
	struct rte_bbdev_enc_op *ops_enq[num_ops];
	struct rte_bbdev_enc_op *ops_deq[num_ops];
		...
		struct test_op_params *op_params)
{
	...
	printf("+ ------------------------------------------------------- +\n");
	printf("== new test with library: throughput\ndev: %s, nb_queues: %u, burst size: %u, num ops: %u, num_lcores: %u, op type: %s, itr mode: %s, GHz: %lg\n",
			info.dev_name, ad->nb_queues, op_params->burst_sz,
			op_params->num_to_process, op_params->num_lcores,
			op_type_str,
			intr_enabled ? "Interrupt mode" : "PMD mode",
